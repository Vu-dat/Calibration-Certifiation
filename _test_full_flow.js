...
const payload = {
  ...
  equipmentName: 'May thử bền màu ma sát'
};
...
const req = http.request({
  ...
  res.on('end', async () => {
    ...
    console.log('RESULT:', j.success ? '✅ SUCCESS' : '❌ FAIL');
    ...
    if (fs.existsSync(fp)) {
      ...
      let foundCount = 0;
      checks.forEach(t => {
        const found = docXml.includes(t);
        console.log(found ? '  ✅' : '  ❌', 'Contains:', t);
        if (found) foundCount++;
      });
      console.log('DATA CHECK:', foundCount + '/' + checks.length, 'fields found');
    } else {
