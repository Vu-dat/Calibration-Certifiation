...
function drawH(doc, logo, cno, cdate, qr, pg) {
  ...
  // Logo - directly above ISO/IEC 17025:2017 at far left, bigger
  if (logo) { try { doc.image(logo, ML, 1, {width:60,height:20}); } catch(e) {} }
  ...
  // Company info block - CENTER-aligned
  sf(doc, true); doc.fontSize(8).fillColor(BCLR);
  doc.text('LabMaster ST Co., Ltd', ML, y, {align:'center',width:CW});
  // Address - center aligned
  sf(doc, false); doc.fontSize(7).fillColor(BCLR);
  doc.text('No.17 street 179, Tang Nhon Phu ward, HCMC', ML, y+12, {align:'center',width:CW});
  // Email/Phone - center aligned
  doc.text('Email: sale@labmaster.vn / Phone: (+84) 938 088 239', ML, y+21, {align:'center',width:CW});
